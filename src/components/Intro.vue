<template>
  ...
  >
    <h3
      class="text-3xl font-extrabold md:text-5xl text-gray-600 font-bricolage uppercase max-w-[1100px] leading-[125%] md:leading-[110%] aos-init aos-animate"
    >
    ...
    <div class="grid grid-cols-3 md:gap-10">
      ...
      <div class="col-span-3 md:col-span-2 space-y-8">
        <p v-for="(item, index) in items" :key="index" class="text-2xl md:text-[32px] leading-[125%] transition-colors duration-700 ease-out text-white">
          {{ item }}
        </p>
      </div>
    </div>
